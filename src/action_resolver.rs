...
fn resolve_action(args: Args) {
    ...
    println!("Action: {:?}, Target format: {:?}", args.action, args.to_format);
    ...
    match args.action {
        ...
        Action::CONVERT => {
            if let Err(e) = convert_action(&path, &args.to_format) {
...
fn convert_action(path: &std::path::Path, target_format: &Format) -> Result<(), Box<dyn std::error::Error>> {
    println!("Performing CONVERT action on: {} to {:?}", path.display(), target_format);
    
    // Read the file first
    let data = file_handler::handle_file(path)?;
    
    // Then convert to target format
    match target_format {
        Format::TXT => convert_to_txt(&data, path)?,
        Format::BIN => convert_to_bin(&data, path)?,
        Format::CSV => convert_to_csv(&data, path)?,
    }
...
fn read_action(path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
